~function(w){
	...
	C("#code-input").on("blur", function(){
		...
		if(this.value === ""){
			...
			C.ajax({
				"type":"post",
				"url":"../data/authCode.php",
				"data":{"code":this.value},
				"fn":codetest
			});
		}
	});	//验证码验证
  C("#auth-pic").click(function () {
		C(this).child().attr("src","../data/codepic.php?r="+new Date().getTime());
  });	//点击图片更换验证码
	C("#upwd-input").on("blur", function(){
		...
		if(C("#verify-input").val() !== "" && this.value !== C("#verify-input").val()){
			C("#verify-input").css("border-color", "#ff6c6c")
				.parent().next().setClass("tips tips-error")
			...
			C("#verify-input").css("border-color", "#45a8ea")
				.parent().next().setClass("tips tips-ok");
			verifyFlag = true;
		}
	});	//密码验证
